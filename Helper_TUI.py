...
def initialize_disk():
	...
	if disk_util == "diskutil": # a bit of a run-on. sorry
		process = subprocess.call(["/usr/bin/sudo", location_diskutil, "partitionDisk", inf_selecteddisk, "4", "APM", "FREE", "%noformat%", "1985S", "FAT32", "%noformat%", str(disk_fat_size), "HFS+", "Installer", "700M", "HFS+", "Root", "0"])
	else:
		process = subprocess.call(["/usr/bin/sudo", location_parted, inf_selecteddisk, "mktable", "mac"])
		process = subprocess.call(["/usr/bin/sudo", location_parted, inf_selecteddisk, "mkpart", "primary", "fat32", "32.8kB", "1048064b"])
		process = subprocess.call(["/usr/bin/sudo", location_parted, inf_selecteddisk, "mkpart", "fat", "fat32", "1048576b", disk_fat_size + "MB"])
		process = subprocess.call(["/usr/bin/sudo", location_parted, inf_selecteddisk, "mkpart", "Installer", "hfs+", disk_fat_size + "MB", str(int(disk_fat_size) + 700) + "MB"])
		process = subprocess.call(["/usr/bin/sudo", location_parted, inf_selecteddisk, "mkpart", "Root", "hfs+", str(int(disk_fat_size) + 700) + "MB", str(disk_size - 1) + "B"])
